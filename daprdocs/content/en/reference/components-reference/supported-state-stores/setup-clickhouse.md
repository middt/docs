...
spec:
  ...
  - name: password # Optional
    value: <PASSWORD>
  # Uncomment this if you wish to use ClickHouse as a state store for actors (optional)
...
If you wish to use ClickHouse as an actor store, append the following to the yaml.
...
|--------------------|:--------:|---------|---------|
| clickhouseUrl      | Y        | Connection URL for the ClickHouse server | `"tcp://localhost:9000"`, `"clickhouse://clickhouse-server:9000"` |
...
```
tcp://[username[:password]@]host[:port][/database][?param1=value1&...&paramN=valueN]
```

Examples:
- `tcp://localhost:9000`
- `tcp://user:password@clickhouse-server:9000/my_db`
